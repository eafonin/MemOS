...
print(f"â Loaded {len(qa_collection)} QA data items")
...
def create_concept_nodes(unique_concepts):
 ...
 print(f"â Created {len(concept_nodes)} concept nodes")
 return concept_nodes

# Execute concept node creation
print("ðï¸ Creating concept nodes...")
...
Answer: {qa_data['final_answer']}"""
 ...
 print(f"â Created {len(qa_nodes)} QA nodes")
...
def create_edges(concept_nodes, qa_nodes, qa_collection):
 ...
 # 1. ConceptâConcept RELATE_TO relationships (derived from Relation QA)
 ...
 print(f"â Created {relate_count} inter-concept RELATE_TO relationships")
 ...
 print(f"â Created {parent_count} ConceptâQA PARENT relationships")
 ...
 print(f"â Created {relation_parent_count} ConceptâBridging QA PARENT relationships")
 print(f"ð Total number of relationships: {len(edges)}")
 
 return edges

# Execute edge relationship creation
print("ð Creating relationship edges...")
...
def assemble_final_json(concept_nodes, qa_nodes, edges):
 ...
 print(f"â Temporary fields cleaned up")
 
 return result

# Execute final assembly
print("ð¦ Assembling final JSON...")
...
def save_final_json(result, filename="cardio_textual_memory_graph.json"):
 ...
 print(f"â Saved to file: {filename}")
 return filename

# Save the results
filename = save_final_json(final_json, "cookbooktest/Cardio/cardio_textual_memory_graph.json")

print("\nð Conversion complete!")
print(f"ð Output file: {filename}")
print(f"ð Final Statistics:")
...
[Now, itâs time to inject this âdigital blueprintâ into a high-performance persistent storage, turning it into a MemCube that can be accessed in real time by MemOS.]
...
def get_driver():
 ...
 print(f"â Failed to create driver: {e}")
 sys.exit(1)
 return driver
...
def test_neo4j_connection():
 ...
 print(f"â Neo4j connection successful: {result.single()['message']}")
 return True
 except Exception as e:
 print(f"â Neo4j connection failed: {e}")
 return False
...
def create_memos_compatible_schema():
 ...
 """)
 print("â Created unique constraint on Memory node ID")
 return True
 
 except Exception as e:
 print(f"â Schema creation failed: {e}")
 return False
...
def bulk_import_nodes():
 ...
 print(f"â Bulk import failed: {e}")
 return success_count
...
def bulk_import_edges():
 ...
 print(f"â Edge import failed: {e}")
 return success_count
...
def create_memos_indexes():
 ...
 print(f"â Index created: {index_query.split()[-7]}") # Extract index name
 ...
 """)
 print("â Vector index created: memory_vector_index (768 dimensions)")
 except Exception as ve:
 print(f"â ï¸ Vector index creation failed: {ve}")
 print(" Vector search functionality will be unavailable")
 print("â All MemOS-compatible indexes created successfully")
 
 except Exception as e:
 print(f"â Index creation failed: {e}")
...
def verify_memos_compatibility():
 ...
 print(f"â MemOS node count: {node_count:,} UserMemory nodes")
 except:
 print("â ï¸ Node counting function needs fine-tuning")
 
 # Test export functionality
 try:
 exported = graph_store.export_graph()
 print(f"â MemOS graph export: {len(exported.get('nodes', []))} nodes, {len(exported.get('edges', []))} edges")
 except Exception as e:
 print(f"â ï¸ Graph export function: {e}")
 
 print("â MemOS compatibility verification complete")
 return True
 
 except Exception as e:
 print(f"â MemOS compatibility verification failed: {e}")
 return False

def main():
 """Main function"""
 print("ð Neo4j Bulk Import Tool")
 print("=" * 50)
 ...
 print(f"ð Import statistics:")
 print(f" Nodes: {node_count:,}")
 print(f" Edges: {edge_count:,}")
 print(f" MemOS compatibility: {'â Fully compatible' if compatible else 'â ï¸ Adjustment needed'}")
 
 if node_count > 0:
 print("\nð¡ You can now use all MemOS features:")
 ...
 print("\nâ User interrupted operation")
 except Exception as e:
 print(f"\nâ Program execution error: {e}")
 finally:
 # Ensure database connection is closed
 config.close_driver()
 print("ð Database connection closed")
...
[Once the data is successfully imported, our cardiovascular MemCube is officially âonline.â In the application, we only need to initialize MemOSâs `TreeTextMemory` through a configuration file pointing to the database. After that, we can interact with the vast knowledge base via the `tree_memory` object, granting the AI specialized domain memory.]
...
print(f"éç½®æä»¶å·²çæ: {json_path}")
...
[To quantify the performance improvements brought by Cardiovascular MemCube, we built an automated evaluation pipeline. The core of this process is: using a powerful third-party model (such as Gemini-2.5-Pro) as a neutral âexaminerâ to verify the improvement in a MemCube-equipped modelâs ability to solve professional problems.]
...
# ð§  MemCube Memory Operating System Medical AI Performance Evaluation Report
...
<table><thead><tr><th>Comparison of Configurations</th><th>MemCube Enhanced Edition Wins</th><th>Baseline Basic Edition Wins</th><th>Draw</th></tr></thead><tbody><tr><td>7B model internal comparison</td><td>47</td><td>3</td><td>150</td></tr><tr><td>32B model internal comparison</td><td>92</td><td>0</td><td>108</td></tr><tr><td>7B+MemCube vs 32B Baseline</td><td>57</td><td>3</td><td>140</td></tr></tbody></table>
 
[Hereâs the full English translation with all original symbols, code, and formatting preserved:]
...
[In contrast, the MemCube-enhanced model (Answer A) demonstrated a higher level of clinical reasoning ability. It was able to invoke and integrate multiple related knowledge fragments (these knowledge pieces were distilled into QA memories for concepts such as âROP,â âcongenital heart disease,â and âsystemic oxygenationâ), for example:]
 
- [Explicitly pointed out that âcongenital heart disease may lead to unstable oxygen delivery, thereby affecting retinal vascular development.â]
- [Emphasized the potential benefits of optimizing âsystemic circulation efficiencyâ for improving ocular vascular health and overall development.]
 
[This reflects the core advantage of MemCube: it is not merely simple information retrieval, but rather the effective association of discrete knowledge points during reasoning, forming a multi-perspective, more in-depth analytical framework. This allows the enhanced model to approach problems like a senior expertâexploring issues from an etiological level and proposing interdisciplinary, integrated management strategies, rather than merely listing examination items.]
...
[However, the MemCube-enhanced model excels further in terms of **structure, logic, and depth of detail**. By retrieving and integrating QA memories in MemCube related to concepts such as âDCM,â âarrhythmia,â and âproarrhythmic risk,â Answer A demonstrates the following characteristics:]
...
- [**Key knowledge points highlighted**: Answer A clearly identifies âoptimized heart failure therapyâ as the foundation for managing arrhythmias and lists drugs such as Î²-blockers, noting their dual role in stabilizing heart rhythm. These originate from QA memories in MemCube that emphasize critical points in clinical practice.]
- [**Stronger risk awareness**: Answer A includes a dedicated section on âProarrhythmic Risk and Management,â showing that the model is not only stating knowledge but also imitating an expertâs risk assessment mindset.]
 
[This case demonstrates that even for a powerful 32B model, MemCube can still serve as a âknowledge coach,â helping the model organize and express its vast internal knowledge in a more structured and clinically logical manner, thereby providing more instructive and professional guidance.]
 
## ð Practical Experience: Try the Cardiovascular Medicine MemCube Demo
...
### ð¦ Features of the Demo System
 
- [**ð« Domain Expertise**: Cardiovascular medicine knowledge system]
- [**ð Data Scale**: 211,315 high-quality memory entries]
- [**ð Association Network**: 522,368 semantic connections between concepts]
- [**ð¾ Data Size**: About 5.0GB of structured medical knowledge]
- [**ð¤ AI Support**: Supports multiple LLM models (GPT-4o, Claude, local models, etc.)]
- [**ð Deployment Ready**: Production-grade architecture based on Neo4j + MemOS]
...
[**ð Cardio MemCube Demo - Hugging Face**]
 
[This demo project provides:]
 
- [â **Complete Installation Guide**: One-click deployment of the Cardiovascular MemCube system]
- [â **Executable Code Examples**: Directly experience the knowledge Q&A functionality]
- [â **Detailed Technical Documentation**: Learn about the construction methodology and best practices]
- [â **Multi-LLM Support**: Flexibly configure different AI model backends]
 
### â ï¸ Important Notes
 
- [**ð¥ Medical Disclaimer**: This demo is for technical demonstration and educational purposes only and should not be used as a basis for medical diagnosis or treatment.]
- [**ð Language Support**: The current version uses an English-optimized embedding model; Chinese queries require translation or replacement with a multilingual embedding model.]
- [**ð§ Technical Framework**: This is a technical reference implementation that can be applied to any professional domain.]
