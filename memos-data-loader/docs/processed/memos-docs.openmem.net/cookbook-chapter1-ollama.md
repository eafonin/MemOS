---
source_url: https://memos-docs.openmem.net/open_source/cookbook/chapter1/ollama
section: Cookbook
...
```
# ð¯ Install Ollama local model service
...
```
# .env
# ð¯ Ollama local model configuration
...
# ð¯ Ollama mode verification script - using local Ollama models and manual configuration
import os
import sys
from dotenv import load_dotenv

def check_ollama_environment():
 """ð¯ Check Ollama environment variable configuration"""
 print("ð Checking Ollama environment variable configuration...")
 ...
 print(f"ð Ollama environment variable status:")
 
 if ollama_base_url:
 print(f" â OLLAMA_BASE_URL: {ollama_base_url}")
 print(f" â OLLAMA_CHAT_MODEL: {ollama_chat_model or 'â Not configured'}")
 print(f" â OLLAMA_EMBED_MODEL: {ollama_embed_model or 'â Not configured'}")
 ollama_configured = bool(ollama_base_url and ollama_chat_model and ollama_embed_model)
 
 if ollama_configured:
 print("â Ollama configuration complete")
 else:
 print("â Ollama configuration incomplete")
 
 return ollama_configured
 else:
 print(f" â OLLAMA_BASE_URL: Not configured")
 print(f" â OLLAMA_CHAT_MODEL: Not configured")
 print(f" â OLLAMA_EMBED_MODEL: Not configured")
 return False
...
def test_ollama_functionality():
 """ð¯ Test Ollama mode functionality"""
 print("\nð Testing Ollama mode functionality...")
 ...
 print("ð Creating Ollama configuration...")
 ...
 print("â Configuration created successfully!")
 
 # Create MOS instance and MemCube
 print("ð Creating MOS instance and MemCube...")
 memory = MOS(mos_config)
 mem_cube = GeneralMemCube(cube_config)
 memory.register_mem_cube(mem_cube)
 
 print("â MOS instance and MemCube created successfully!")
 ...
 print("\nð§ Testing MemCube direct operations...")
 ...
 print("â MemCube direct operations successful!")
 
 print("â Ollama mode functionality test successful!")
 return True
 
 except Exception as e:
 print(f"â Ollama mode functionality test failed: {e}")
 print("ð¡ Tip: Please check if Ollama service is running and models are downloaded.")
 return False

def main():
 """ð¯ Ollama mode main verification process"""
 print("ð Starting MemOS Ollama mode environment verification...\n")
 ...
 func_ok = False
 if not env_ok:
 print("\nâ ï¸ Skipping functionality test due to incomplete Ollama environment variable configuration")
 elif not install_ok:
 print("\nâ ï¸ Skipping functionality test due to MemOS installation failure")
 
 # Summary
 print("\n" + "="*50)
 print("ð Ollama mode verification results summary:")
 print(f" Ollama environment variables: {'â Passed' if env_ok else 'â Failed'}")
 ...
 print(f"\nð Congratulations! MemOS Ollama mode environment configuration completely successful!")
 print(f"ð¡ You can now start using MemOS Ollama mode.")
 print(f"ð¡ Usage: Manual configuration of MOSConfig and GeneralMemCubeConfig")
 elif install_ok and env_ok:
 print(f"\nâ ï¸ MemOS is installed and Ollama is configured, but functionality test failed.")
 print(f"ð¡ Please check if Ollama service is running and models are downloaded.")
 elif install_ok:
 print("\nâ ï¸ MemOS is installed, but need to configure Ollama environment variables to use normally.")
 print("ð¡ Please configure OLLAMA_BASE_URL, OLLAMA_CHAT_MODEL, OLLAMA_EMBED_MODEL in .env file.")
 else:
...
```
# ð§ GPU acceleration only needed when using Ollama local models
...
[**ð¡ Ollama Mode:** This script uses local Ollama models for document processing and vectorization.]
 
[**ð§ Environment Variable Configuration:** Before running the script, please ensure you have configured Ollama environment variables according to Recipe 1.1.]
 
[For usage of other parsers, please refer to documentation: TODO]
 
```
# create_memcube_with_memreader_ollama.py
# ð¯ Complete process of creating MemCube using MemReader (Ollama version)
import os
import uuid
from dotenv import load_dotenv
from memos.configs.mem_cube import GeneralMemCubeConfig
...
def create_memreader_config():
 """
 ð¯ Create MemReader configuration (Ollama version)
 """
...
def load_document_to_memcube(mem_cube, doc_path):
 """
 ð¯ Load document to MemCube using MemReader (Ollama version)
 """
 ...
 print(f" ð Embedding model: {os.getenv('OLLAMA_EMBED_MODEL')} (Ollama)")
 print(f" ð¯ Configuration mode: OLLAMA")
...
if __name__ == "__main__":
 print("ð Starting to create document MemCube using MemReader (Ollama version)...")
...
[**ð¡ Ollama Mode:** This script uses local Ollama models for search and chat.]
...
# ð¯ Test MemCube search and chat functionality (Ollama version)
import os
from dotenv import load_dotenv
from memos.configs.mem_os import MOSConfig
from memos.mem_os.main import MOS
...
def create_mos_config():
 """
 ð¯ Create MOS configuration (Ollama version)
 """
 load_dotenv()
...
def test_memcube_search_and_chat():
 ...
 print("ð Starting to test MemCube search and chat functionality (Ollama version)...")
 ...
 print(f" ð¯ Configuration mode: OLLAMA")
 print(f" ð¤ Chat model: {os.getenv('OLLAMA_CHAT_MODEL')} (Ollama)")
 print(f" ð Embedding model: {os.getenv('OLLAMA_EMBED_MODEL')} (Ollama)")
...
[**ð§ Solution:** Master the complete lifecycle management of MemCube, including environment detection, intelligent configuration, basic queries, advanced metadata operations, and fine-grained memory and file management.]
 
#### Step 1: Complete MemCube Lifecycle Management
 
```
 # memcube_lifecycle_ollama.py
# ð¯ MemCube lifecycle management: create, add memory, save, load, query, delete (Ollama version)
import os
import shutil
import time
...
class MemCubeManager:
 """
 ð¯ MemCube lifecycle manager (Ollama version)
 """
...
def demonstrate_lifecycle():
 ...
 print("ð Starting MemCube lifecycle demonstration...\n")
...
if __name__ == "__main__":
 """
 ð¯ Main function - run MemCube lifecycle demonstration
 """
 try:
